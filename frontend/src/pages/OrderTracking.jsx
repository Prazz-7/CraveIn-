...
const LABELS = {
  pending_confirmation: "Pending Restaurant Confirmation",
  confirmed: "Confirmed",
  preparing: "Preparing",
  ready: "Ready",
...
export default function OrderTracking() {
  ...
  try {
    ...
    load(); // Refresh the order after updating
  } catch (err) {
    console.error(err);
  }
};
  ...
  useEffect(() => {
    ...
    const waitingForRestaurant = status === "pending_confirmation";
    // Stop polling for final states: delivered, cancelled, or rejected
    if (status === "delivered" || status === "cancelled" || status === "rejected") return;
  ...
  return (
    <div
      className="container"
      style={{ paddingTop: "2rem", paddingBottom: "3rem", maxWidth: 840 }}
    >
      <button
      ...
      >
        <div>
          ...
          >
            {isRejected ? "Your order was rejected" : "Your Order"}
            {isActive && (
            ...
            {!isRejected && isCancelled && <span className="badge badge-red">Cancelled</span>}
          </h1>
      ...
      {/* {waitingForRestaurant && (
        ...
        >
          ⏳ Your order has been sent to the restaurant and is waiting for
          confirmation.
        </div>
      )} */}
          ...
          <div style={{ padding: "2rem", background: "#fafaf8" }}>
            {isCancelled ? (
              <div style={{ textAlign: "center", padding: "1.5rem 0" }}>
                ...
                <h3 style={{ color: "#dc2626" }}>{isRejected ? "Order Rejected" : "Order Cancelled"}</h3>
              </div>
            ) : (
                  ...
                  {PIPELINE.map((s, idx) => {
                    ...
                    return (
                      <div key={s} className="step">
                        ...
                        <span
                          className={`step-label${done ? " done" : current ? " current" : ""}`}
                        >
                          {LABELS[s]}
                        </span>
                      </div>
    ...
    >
      ...
      {isRejected && (
        ...
        >
          ❌ Your order was rejected by the restaurant. Please place a new order.
        </div>
      )}
      ...
      <div className="grid-sidebar">
        <div className="card">
          ...
          {(() => {
            const grouped = (order.items || []).reduce((acc, item) => {
              ...
              if (!acc[key])
                acc[key] = { name: item.restaurant_name, items: [] };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          >
            ...
            >
              <span>Subtotal</span>
              <span>NPR {order.total_amount - order.delivery_fee}</span>
            </div>
            <div
              style={{
                display: "flex",
